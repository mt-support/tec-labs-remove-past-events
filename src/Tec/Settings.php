...
class Settings {
	...
	private function set_options_prefix( $options_prefix = '' ) {
		if ( empty( $opts_prefix ) ) {
			$opts_prefix = str_replace( '-', '_', 'tec-labs-remove-past-events-plus' ); // The text domain.
		}
	...
	public function add_settings() {
		$fields = [
			...
			'a_setting' => [ // TODO: Change setting.
				'type'            => 'text',
				'label'           => esc_html__( 'Example setting', 'tec-labs-remove-past-events-plus' ),
				'tooltip'         => sprintf( esc_html__( 'Example setting description. Enter your custom URL, including "http://" or "https://", for example %s.', 'tec-labs-remove-past-events-plus' ), '<code>https://demo.theeventscalendar.com/</code>' ),
	...
	private function get_example_intro_text() {
		$result = '<h3>' . esc_html_x( 'Example Extension Setup', 'Settings header', 'tec-labs-remove-past-events-plus' ) . '</h3>';
		$result .= '<div style="margin-left: 20px;">';
		$result .= '<p>';
		$result .= esc_html_x( 'Some text here about this settings section.', 'Setting section description', 'tec-labs-remove-past-events-plus' );
		$result .= '</p>';
		$result .= '</div>';
